...
int  main (int argc, char* argv[])  {
  ...
  n = score.size();  // number of input matches
  ...
  for  (i = 1, j = 0;  i < n;  i++) {
    if  (score[j].x == score[i].x && score[j].y == score[i].y)  {
      ...
      j++;
      if  (i != j) {
	score[j] = score[i];  // this is for elements deletion purpose
      }
    }
  }
  
  j++;
  score.resize (j); //  save RAM
    /*
  ...
  //  it as a DAG
    ...
    do {
        ...
        for (j = 1; j < n; j++) {
            for (i=j-1; i >= 0; i--) {
                int  del_x, del_y;
                del_x = score[j].x - score[i].x - 1;  // this is the position information
                del_y = score[j].y - score [i].y - 1;  // this is the position information
                if  (del_x >= 0 && del_y >= 0)  {
