...
void alignNeedlemanForTranscript_simd_avx2int32::calculate_similarity( __m256i** vProfile, NucleotideCodeSubstitutionMatrix& nucleotideCodeSubstitutionMatrix){
    ...
    __m256i* vP;
    __m256i e;
    __m256i vGapO, vGapE, next_vGapO, next_vGapE;
    int z;
    int32_t i, j, k;
    int32_t * a;
    int32_t * b;
    ...
    for (i = 0; i<_length_of_d; ++i) {//important
        ...
        for (k = 0; k < 8; ++k) {
            vF = _mm256_alignr_epi8(vF, _mm256_permute2x128_si256(vF, vF, _MM_SHUFFLE(0, 0, 2, 0)), 16 - 4);
            a = (int32_t *) (&vF);
            a[0]=_extend_gap_penalty[_ref_elements[i]];
            for (j = 0; j < this->_segLen; ++j) {
                vH = _mm256_loadu_si256(pvHStore + j);
        ...
        for (k = 0; k < 8; ++k) {
            ...
            for (j = 0; j < this->_segLen; ++j) {
                ...
                vH = _mm256_add_epi32(vH, vGapO);
                vF = _mm256_add_epi32(vF, vGapE);
//                if(UNLIKELY(!_mm256_movemask_epi8(_mm256_cmpgt_epi16(vF, vH)))) goto end_loop_i16;
            }
        }
        //end_loop_i16:
        ...
        //update with track matrices _track_ins begin
        for( j=0; j<_length_of_q; ++j ){
            if( _trace_ins[j+1][i+1] && _trace_match[j+1][i+1]==false && !_trace_del[j+1][i+1] ){
                t=0;
                while( i>=t && _trace_ins[j+1][i-t] && _trace_match[j+1][i-t] ){
                    _trace_match[j+1][i-t] = false;
                    ++t;
                }
                t=0;
                while( i>=t && _trace_ins[j+1][i-t] && _trace_del[j+1][i-t] ){
                    _trace_ins[j+1][i-t] = false;
                    ++t;
                }
            }
