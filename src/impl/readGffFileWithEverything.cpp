...
/**
 * the following two functions are used to reads the gff file with duplication gene ids and transcripts id
 * for example the output of liftover annotation for de novo assembly genome sequence
 * */



void readGffFileWithEveryThing (const std::string& filePath, std::map<std::string, std::vector<Gene> > & geneMap){
    std::set<std::string> cdsParentRegex;
    std::set<std::string> transcriptParentRegex;
    std::set<std::string> transcriptIdRegex;
    std::set<std::string> geneIdRegex;
...
void readGffFileWithEveryThing (const std::string& filePath, std::map<std::string, std::vector<Gene> > & geneMap,
                ...
                if (geneNickNames.find(elemetns[2]) != geneNickNames.end()) {
                    ...
                    for (std::regex regGene : regGeneIds) {
                        ...
                        if (!match.empty()) {
                            ...
                            geneMap[chromosomeName][geneMap[chromosomeName].size()-1].setStart(start);
                            geneMap[chromosomeName][geneMap[chromosomeName].size()-1].setEnd(end);
                            geneMap[chromosomeName][geneMap[chromosomeName].size()-1].setSource(source);
                            geneMap[chromosomeName][geneMap[chromosomeName].size()-1].setScore(score);
                            geneMap[chromosomeName][geneMap[chromosomeName].size()-1].setLastColumnInformation(lastColumnInformation);
                            matched = true;
                            break; //jump out for loop
                        }
                    }
                    ...
                    for (std::regex regTranscriptParent : regTranscriptParents) {
                        ...
                        if (!match.empty()) {
                            ...
                            if( geneId.compare(geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getName()) == 0 ){
                                ...
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].setEnd(end);
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].setSource(source);
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].setScore(score);
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].setLastColumnInformation(lastColumnInformation);
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].setType(elemetns[2]);
                            }
                            matched = true;
                            break; // jump out for loop
                        }
                    ...
                    for (std::regex regCdsParent : regCdsParents) {
                        ...
                        if (!match.empty()) {
                            ...
                            if( transcriptId.compare(geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].getName()) == 0 ){
                                GenomeBasicFeature cds(start, end, score, elemetns[7], lastColumnInformation);
                                cds.setType(elemetns[2]);
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].addCds(cds);
                                //                        std::cout << transcriptId << "adding cds" << std::endl;
                            }
                            matched = true;
                            break; // jump out for loop
                    ...
                    for (std::regex regCdsParent : regCdsParents) {
                        ...
                        if (!match.empty()) {
                            ...
                            if( transcriptId.compare(geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].getName()) == 0 ){
                                GenomeBasicFeature five_prime_utr(start, end, score, elemetns[7], lastColumnInformation);
                                five_prime_utr.setType(elemetns[2]);
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].addFivePrimerUtr(five_prime_utr);
                            }
                            matched = true;
                            break; // jump out for loop
                        }
                    ...
                    for (std::regex regCdsParent : regCdsParents) {
                        ...
                        if (!match.empty()) {
                            ...
                            if( transcriptId.compare(geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].getName()) == 0 ){
                                GenomeBasicFeature three_prime_utr(start, end, score, elemetns[7], lastColumnInformation);
                                three_prime_utr.setType(elemetns[2]);
                                geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts()[geneMap[chromosomeName][geneMap[chromosomeName].size()-1].getTranscripts().size()-1].addThreePrimerUtr(three_prime_utr);
                            }
                            matched = true;
                            break; // jump out for loop
                        }
    ...
    for (std::map<std::string, std::vector<Gene>>::iterator it1=geneMap.begin(); it1!=geneMap.end(); ++it1){
        for( std::vector<Gene>::iterator it=it1->second.begin(); it!=it1->second.end(); ++it){
            for( std::vector<Transcript>::iterator transcriptIt = it->getTranscripts().begin();
                 transcriptIt != it->getTranscripts().end();
                 ++transcriptIt){
                if( (*transcriptIt).getCdsVector().size()>0 ){
                    (*transcriptIt).updateInforCds();
                }
            }
        }
    }
